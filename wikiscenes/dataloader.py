...
CAT_LIST = ['portal', 'window', 'tower']

N_CAT = len(CAT_LIST)

CAT_NAME_TO_NUM = dict(zip(CAT_LIST,range(len(CAT_LIST))))

# cls_labels_dict = np.load('wikiscenes/cls_labels.npy', allow_pickle=True).item()
...
def load_image_label_list_from_npy(img_name_list):
    ...
    for f in img_name_list:
        for i in range(len(CAT_LIST)):
            if f.split('/')[0] == CAT_LIST[i]:
                label = torch.zeros(len(CAT_LIST))
                label[i] = 1
...
def load_img_name_list(dataset_path):
    ...
    for file in dataset_path:
        img_name_list.append(file.split('/')[1])
...
class TorchvisionNormalize():
    ...
    def __call__(self, img):
        ...
        proc_img = np.float32(imgarr)
        proc_img[..., 0] = imgarr[..., 0] / 255.
        proc_img[..., 1] = imgarr[..., 1] / 255.
        proc_img[..., 2] = imgarr[..., 2] / 255.
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        name_str = name
        # try:
        img_path = os.path.join(self.voc12_root, self.img_name_list_path[idx])
        img = np.asarray(imageio.imread(img_path))
        # except:
        #     img = np.asarray(np.zeros((588,588,3)))
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,
        ...
        self.label_list = load_image_label_list_from_npy(self.img_name_list_path)
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __getitem__(self, idx):
        name = self.img_name_list_path[idx]
        name_str = name
        img_path = os.path.join(self.voc12_root, name)
        img = np.asarray(imageio.imread(img_path))
